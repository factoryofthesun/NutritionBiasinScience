# -*- coding: utf-8 -*-
"""
Include grains tagged data in XLNet model training.
This version includes unrelated under the "non-positive" tag.
TODO: DEAL WITH IMBALANCED CLASSES
...
oats_df = pd.read_csv(data_path + "/mturk_train.csv")

#Polarity rule: If >=2 positive ratings, then label positive
oats_df['polarity'] = (oats_df['count_pos'] >= 2).astype(int)

oats_abstracts = oats_df['inputtext'].tolist()
oats_labels = oats_df['polarity'].tolist()

# Load grains data
grains_df = pd.read_csv(data_path + "/grains_formatted.csv")
grains_df['polarity'] = (grains_df['Positive'] >= 2).astype(int)

grains_abstracts = grains_df['Input.text'].tolist()
grains_labels = grains_df['polarity'].tolist()

abstracts = oats_abstracts + grains_abstracts
labels = oats_labels + grains_labels

assert len(abstracts) == len(labels)

positive_ratio = len([x for x in labels if x == 1])/len(labels)
print(f"Total data positivity ratio: {positive_ratio}")
...
def preProcess(max_len, abstracts, labels, function):
    # Employ specified sampling technique
    if function == 'RandomOverSampler':
        print("Executing random over sampler...")
        ros = RandomOverSampler(random_state = 0)
        abs_resampled, labels_resampled = ros.fit_resample(np.reshape(abstracts, (-1,1)), labels)
        abs_resampled = abs_resampled.flatten()
        print(sorted(Counter(labels_resampled).items()))
    elif function == 'RandomUnderSampler':
        print("Executing random under sampler...")
        ros = RandomUnderSampler(random_state = 0)
        abs_resampled, labels_resampled = ros.fit_resample(np.reshape(abstracts, (-1,1)), labels)
        abs_resampled = abs_resampled.flatten()
        print(sorted(Counter(labels_resampled).items()))
    else:
        # Keep original imbalance
        abs_resampled, labels_resampled = abstracts, labels
    #Tokenize
    abs_tokens = [tokenizer.tokenize(ab) + ["<sep>", "<cls>"] for ab in abs_resampled]
    ...
    train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = train_test_split(input_ids, labels_resampled,
...
plots_path = str(Path(__file__).parent / "../Plots/Grains")
def plotResults(train_loss_set, train_acc_set, test_loss_set, test_acc_set):
    loss_fname = f"/loss_{function}.png"
    acc_fname = f"/accuracy_{function}.png"
...
# Loop through different imbalanced sampling techniques
sampling_functions = ["original", "RandomOverSampler", "RandomUnderSampler"]

for function in sampling_functions:
    #Set seed
    seed_val = 2
    ...
    maxlen = 128
    #Preprocess
    train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = preProcess(maxlen, abstracts, labels, function)
    ...
    torch.save(model.state_dict(), model_path + f"/grains_{function}_AdamW_dp0.3_sdp0.1_bsize128.pt")
