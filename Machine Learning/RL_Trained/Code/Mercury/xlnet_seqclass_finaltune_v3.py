...
def plotResults(train_loss_set, train_acc_set, test_loss_set, test_acc_set):
    loss_fname = "/mlen_{}_loss_final.png".format(max_len)
    acc_fname = "/mlen_{}_accuracy_final.png".format(max_len)
...
#Robustness check: train model 5 times

for max_len in [512, 800]:
    seed_val = 0
    random.seed()
    np.random.seed()
    #torch.manual_seed()
    ...
    warmup = 0.1
    #Preprocess
    train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = preProcess(max_len, abstracts, labels)
    ...
    epochs = 50 #Solved overfitting with gradient accumulation - now implement early stopping
