...
def kfoldpreProcess(max_len, abstracts, labels):
    ...
    # Get stratified 10-Fold indices
    input_tensors_list = []
    kf = StratifiedKFold(n_splits= 10)
    for train_ind, test_ind in kf.split(input_ids, labels):
...
# Use RayTune with distributed execution to hyperparameter tune
import ray
...
def raytrain(config):
    ...
    summary_dropout = config['summary_dropout']
    learning_rate = config['lr']
    layerdecay = config['layerdecay']
    eps = 1e-8
    optim = config['optim']
    warmup = 0.1
    maxlen = 128
    ...
    # Cross validate over the chosen hyperparameters
    validation_accs = []
    ...
    track.log(mean_accuracy=mean(validation_accs))

    # Save model into trial directory
    torch.save(model.state_dict(), "./xlnet_ray.pt")

search_space = {
    "optim": tune.grid_search(["AdamW", "RMSprop"]),
    "dropout": tune.sample_from(lambda spec: np.random.uniform(0, 0.3)),
    "summary_dropout": tune.sample_from(lambda spec: np.random.uniform(0, 0.3)),
    "lr": tune.sample_from(lambda spec: np.random.uniform(1e-5, 4e-5)),
    "layerdecay": tune.grid_search([True, False])
}

analysis = tune.run(raytrain, num_samples = 5,
                    scheduler=ASHAScheduler(metric="mean_accuracy", mode="max"),
                    config=search_space)
print("Best config is", analysis.get_best_config(metric="mean_accuracy"))
print("Best config directory is", analysis.get_best_logdir("mean_accuracy", mode="max"))

import os
diagnostics_path = str(Path(__file__).parent / "../Diagnostics")

df = analysis.dataframe() # Dataframe of last reported results in all trials
df.to_csv(diagnostics_path + "/xlnet_raytune.csv", index = False)
