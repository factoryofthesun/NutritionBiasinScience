...
length(unique(abstract_dt$SO))

# Percentages by publication 
abstract_grouped <- abstract_dt[,.(Counts = .N), by = list(SO, Is_Industry)]
...
abstract_grouped$SO_ID <- as.numeric(abstract_grouped$SO)

# Plot histograms 
ggplot(abstract_grouped, aes(x=SO_ID, y=Perc_Industry_Status)) + geom_bar(stat="identity") + 
...
top3 <- top_journals$SO[1:3]
top3_abstracts <- abstract_dt[SO %in% top3]
top3_abstracts <- top3_abstracts[order(SO),.(Perc_Positive = sum(Prediction)/.N), by=list(SO, Is_Industry)]

# Journal of Dairy Science has large industry source proportion - check food groups that submit to it
dscience_abstracts <- abstract_dt[SO=="JOURNAL OF DAIRY SCIENCE"]
...
ind_plot <- ggplot(industry_abstracts, aes(x=SO)) + geom_bar(aes(y=(..count..)/sum(..count..))) + 
  scale_y_continuous(labels=scales::percent)

nonindustry_abstracts <- abstract_dt[Is_Industry == 0]
nonind_plot <- ggplot(nonindustry_abstracts, aes(x=SO)) + geom_bar(aes(y=(..count..)/sum(..count..))) + 
  scale_y_continuous(labels=scales::percent)
...
journal_distribution <- journal_distribution[,.(Counts=.N), by=list(SO,Is_Industry)]
...
write.xlsx(journal_wide, "Distribution_Analysis/pub_journal.xlsx", row.names=F)
